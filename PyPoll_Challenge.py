...
#challenge objects created below 
county_votes_dict = {}
County_name_lis = []
...
#challenge for loop to create and fill dictionary of county names and votes
with open(file_to_load) as election_data:
    file_reader = csv.reader(election_data)
    for row in file_reader:
        county_name = row[1]
        if county_name not in County_name_lis:
            # add county name to the county list. 
            County_name_lis.append(county_name)
            # being tracking the county each vote comes from.
            county_votes_dict[county_name] = 0
            #add a count to the value of the county the vote came from.
        county_votes_dict[county_name] += 1
           
#print("county_votes_dict :",county_votes_dict)
